...
namespace Mabrouk\ProjectSetting\Http\Middleware;

use Closure;
use Illuminate\Support\Str;

class ProjectSettingOfficerMiddleware
{
    ...
    public array $subProjectSettingsNames;

    public function __construct()
    {
        $this->baseUrls = config('permissions.base_urls') ?? [];
        $this->excludedRoutes = config('permissions.excluded_routes') ?? [];
        $this->subProjectSettingsNames = $this->authUser()?->subProjectSettingsNames ?? [];
    }
    ...
    {
        $routeSubProjectSettingName = $this->subProjectSettingNameFromRoute(
            $this->sanitizeBaseUrl($request->route()->uri()),
            $request->route()->methods()[0]
        );
        if (
            (
                $this->routeIsProjectSettingable($request->route()->uri())
                && ! (bool) $this->authUser()
            ) || (
                $routeSubProjectSettingName != null
                && ! \in_array($routeSubProjectSettingName, $this->subProjectSettingsNames)
                && $this->routeIsProjectSettingable($request->route()->uri())
            )
        ) {
    ...
    private function subProjectSettingNameFromRoute(string $url, $routeMethod) : string
    {
     ...
     */
    public function routeIsProjectSettingable($uri) : bool
    {
